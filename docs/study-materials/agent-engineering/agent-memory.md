...
### Project 1：Agent Memory Prototype

- 实现短期记忆、长期记忆和反思记忆三类存储。
- 给每条记忆记录来源、时间、重要性、过期策略和隐私级别。
- 支持写入、检索、更新、删除四类操作。

完成标准：

- 能在 30-50 个多轮任务上复现记忆写入和召回；
- 记录 memory hit rate、污染率、过期清理比例；
- 输出一份失败分析，说明问题来自检索、写入策略还是反思摘要。

### Project 2：Memory Evaluation Harness

- 构造需要长期状态的任务集，例如偏好记忆、跨会话项目状态、工具使用历史。
- 对比 no memory、summary memory、vector memory、hybrid memory。
- 评估任务成功率、一致性、隐私泄漏风险和 token 成本。

完成标准：

- 至少比较 3 种 memory 策略；
- 给出推荐架构和不适合使用 memory 的边界条件。
